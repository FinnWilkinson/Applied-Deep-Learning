...
def main(args):
    if args.data_aug_hflip:
        test_transform = transforms.Compose([
            ...
            transforms.RandomRotation(degrees=args.data_aug_rotation),
            transforms.ToTensor()
        ])
    else:
        test_transform = transforms.Compose([
...
class CNN(nn.Module):
    def __init__(self, height: int, width: int, channels: int, class_count: int, dropout: float):
        ...
        self.batchNorm2d_1 = nn.BatchNorm2d(self.conv1.out_channels)
        self.batchNorm2d_2 = nn.BatchNorm2d(self.conv2.out_channels)
...
class Trainer:
    ...
    def validate(self):
        ...
        print(f"validation loss: {average_loss:.5f}, accuracy: {accuracy * 100:2.2f}, per Class Accuracies: {class_accuracies}")
...
def compute_per_class_accuracy(labels: Union[torch.Tensor, np.ndarray], preds: Union[torch.Tensor, np.ndarray]) -> float:
    ...
    """
    assert len(labels) == len(preds)
    class_accuracies = [0.0] * 10
    for c in range (0,10):
        labelsMask = (labels == c)
        predsMask = (preds == c)
        labelsInC = labelsMask.sum()  #number of labels equal to class
        correctPreds = (labelsMask * predsMask).sum()
        class_accuracies[c] = float(correctPreds) / (labelsInC)
